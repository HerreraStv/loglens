...
def test_main_with_level_filters_output(tmp_path, capsys):
    log_file = tmp_path / "sample.log"
    log_file.write_text(
        "2026-01-01T00:00:00Z INFO api hello\n"
        "2026-01-01T00:00:01Z WARNING api careful\n"
    ...
    exit_code = main([str(log_file), "--level", "ERROR"])
...
def test_main_level_is_case_insensitive(tmp_path, capsys):
    log_file = tmp_path / "sample.log"
    log_file.write_text("2026-01-01T00:00:00Z ERROR api boom\n", encoding="utf-8")
...
def test_main_rejects_invalid_level(tmp_path, capsys):
    log_file = tmp_path / "sample.log"
    log_file.write_text("2026-01-01T00:00:00Z ERROR api boom\n", encoding="utf-8")

    with pytest.raises(SystemExit) as exc_info:
        main([str(log_file), "--level", "DEBUG"])
